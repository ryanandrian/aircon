/**
 * API /api/assets/[id] — get+riwayat (GET), update (PATCH), soft delete (DELETE).
...
export async function GET(
  ...
  try {
    const asset = await getAssetWithHistory(tenantId, id);
...
export async function PATCH(
  ...
  try {
    const input = updateAssetSchema.parse(json);
    const asset = await updateAsset(tenantId, id, input);
    return NextResponse.json({ data: asset });
  } catch (err) {
...
export async function DELETE(
  ...
  const { id } = await ctx.params;
  try {
    const asset = await softDeleteAsset(tenantId, id);
    return NextResponse.json({ data: asset });
  } catch (err) {
    return handleServiceError(err);
  }
