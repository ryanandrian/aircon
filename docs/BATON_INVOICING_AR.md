...
- **K5b** GERBANG PROGRAM INSENTIF (NIAT eksplisit): `Tenant.incentiveEnabled` (Boolean, default **false**). Tenant BARU default OFF (aman — aplikasi TIDAK memperkenalkan ekspektasi insentif ke teknisi). Owner mengaktifkan di Pengaturan → Insentif Tim. **GERBANG DITEGAKKAN DI SUMBER**: `computeIncentives()` return `[]` dan `listTechnicianJobHistory()` set semua insentif 0 + kembalikan flag `incentiveEnabled` bila OFF — jadi SEMUA pemanggil (owner laporan, dashboard teknisi, ganti-filter riwayat, pemanggil baru) patuh otomatis tanpa cek per-call-site. OFF → teknisi TAK melihat kata/kartu/kolom/angka insentif sama sekali. Bukan ditebak dari nilai katalog (nilai 0 ambigu). Migrasi tenant lama: incentiveEnabled=true bila sudah punya layanan insentif>0.
